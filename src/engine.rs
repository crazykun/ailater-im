...
impl InputEngine {
    ...
    pub fn handle_key(
        ...
        let result = match key {
            ...
            KeySym::Up => self.handle_up(&mut input_state),
            KeySym::Down => self.handle_down(&mut input_state),
    ...
    fn handle_right(&self, state: &mut InputState) -> IMReturnValue {
        ...
        if state.cursor_pos < state.preedit.len() {
            state.cursor_pos += 1;
        }
    ...
    fn add_phrase_candidates(&self, state: &mut InputState, syllables: &[String]) {
        ...
        // Generate combinations from individual syllables
        if state.candidates.is_empty() && syllables.len() <= 4 {
    ...
    fn generate_combinations(&self, state: &mut InputState, syllables: &[String]) {
        // Get candidates for each syllable
        let all_candidates: Vec<Vec<&str>> = syllables
            .iter()
            .map(|s| get_candidates(s))
            .collect();
        ...
        if syllables.len() == 2 {
            for i in 0..all_candidates[0].len().min(max_per_syllable) {
                for j in 0..all_candidates[1].len().min(max_per_syllable) {
                    ...
                    combinations.push((text, 1.0 - (i + j) as f32 * 0.1));
                }
            }
            ...
            for i in 0..all_candidates[0].len().min(max_per_syllable) {
                for j in 0..all_candidates[1].len().min(max_per_syllable) {
                    for k in 0..all_candidates[2].len().min(max_per_syllable) {
                        ...
                        combinations.push((text, 1.0 - (i + j + k) as f32 * 0.1));
                    }
                }
            }
        }
        
        // Add combinations as candidates
